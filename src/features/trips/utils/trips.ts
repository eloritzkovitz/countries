...
/**
 * Gets visited countries grouped by year.
 * @param trips - Array of trips to analyze.
 * @returns A record of visited countries grouped by year.
 */
export function getVisitedCountriesByYear(trips: Trip[]) {
  const visits: Record<number, Set<string>> = {};
  for (const trip of trips) {
    // Use startDate or endDate for the year (prefer startDate)
    const date = trip.startDate || trip.endDate;
    if (!date) continue;
    const year = new Date(date).getFullYear();

    if (!visits[year]) visits[year] = new Set();
    if (trip.countryCodes) {
      trip.countryCodes.forEach((iso) => visits[year].add(iso));
    }
  }
  return visits;
}
...
/**
 * Gets all countries visited up to and including a given year.
 * @param trips - Array of trips to analyze.
 * @param year - The year up to which to include trips.
 * @returns A Set of country codes visited up to the given year.
 */
export function getVisitedCountriesUpToYear(
  ...
  for (const trip of trips) {
    ...
    if (tripYear <= year && trip.countryCodes) {
      trip.countryCodes.forEach((iso) => visited.add(iso));
    }
  }
  return visited;
}
